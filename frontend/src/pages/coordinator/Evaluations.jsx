    ...
    useEffect(() => {
        const fetchStudents = async () => {
            try {
                ...
                setStudents(list)
                // Auto-select from query param or first student
    ...
    return (
        <div className="fade-in max-w-5xl mx-auto space-y-6 pb-12">
            <div className="mb-6">
                ...
                <p className="page-sub mt-1">View OJT performance evaluations submitted by supervisors.</p>
            </div>

            {/* Info banner */}
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-indigo-900/10 border border-indigo-800/30 text-xs text-indigo-400">
                <span>ℹ️</span>
                <span>Evaluations are submitted by assigned supervisors only, after students complete their required OJT hours.</span>
            </div>
            ...
            {selectedStudent && (
                loadingEvals ? (
                    ...
                    <div className="card text-center py-12">
                        ...
                        <p className="text-xs text-slate-600 mt-1">
                            {selectedStudent.status === 'Completed'
                                ? 'This student has completed their hours. The supervisor can now submit an evaluation.'
                                : `Student is at ${selectedStudent.progress_pct}% of required hours — evaluation is unlocked at 100%.`}
                        </p>
                    </div>
                ) : (
                    <div className="space-y-6">
                        {/* Summary card */}
                        {summary && (
                            <div className="card bg-gradient-to-br from-indigo-900/20 to-slate-900/50 border-indigo-800/30">
                                <h2 className="text-xs font-bold uppercase tracking-widest text-indigo-400 mb-4">Average Across {summary.total_evaluations} Evaluation{summary.total_evaluations !== 1 ? 's' : ''}</h2>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-4">
                                    {JOB_FACTORS.map(f => (
                                        <FactorScore
                                            key={f.key}
                                            label={f.label}
                                            score={summary[`avg_${f.key}`] ?? summary[`avg_${f.key.replace('company_rules_observance', 'company_rules')}`] ?? 0}
                                            max={f.max}
                                        />
                                    ))}
                                </div>
